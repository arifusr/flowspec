...
function activate(context) {
    index = new indexer_1.FlowSpecIndex();
    index.buildIndex();
    ...
    const definitionProvider = vscode.languages.registerDefinitionProvider(selector, new FlowSpecDefinitionProvider(outputChannel));
    context.subscriptions.push(definitionProvider);
    // Hover
    const hoverProvider = vscode.languages.registerHoverProvider(selector, new FlowSpecHoverProvider());
    context.subscriptions.push(hoverProvider);
    // Reference Provider — find all usages of a request
    const referenceProvider = vscode.languages.registerReferenceProvider(selector, new FlowSpecReferenceProvider());
    context.subscriptions.push(referenceProvider);
}
function deactivate() { }
class FlowSpecDefinitionProvider {
    ...
    provideDefinition(document, position) {
        ...
        if (def) {
            ...
            return new vscode.Location(vscode.Uri.file(def.file), new vscode.Position(def.line, 0));
        }
        this.output.appendLine(`  → NOT FOUND in index`);
        return null;
    }
}
class FlowSpecHoverProvider {
...
function getTargetAtPosition(line, character) {
...
function getFileReference(line) {
    ...
    if (includeMatch)
        return includeMatch[1].trim();
    return null;
}
function resolveFilePath(currentUri, relativePath) {
    const fs = require('fs');
...
function formatDefinitionPreview(def) {
                ...
                if (entry.isDirectory()) {
                    ...
                    for (let i = 0; i < lines.length; i++) {
                        ...
                        if (regex.test(lines[i])) {
                            ...
                            locations.push(new vscode.Location(vscode.Uri.file(fullPath), new vscode.Position(i, col >= 0 ? col : 0)));
                        }
                    }
                }
            }
...
class FlowSpecReferenceProvider {
                ...
                for (const entry of entries) {
                    ...
                    if (entry.isDirectory()) {
                        ...
                        for (let i = 0; i < lines.length; i++) {
                            ...
                            if (regex.test(lines[i])) {
                                locations.push(new vscode.Location(vscode.Uri.file(fullPath), new vscode.Position(i, lines[i].indexOf(name))));
                            }
                        }
                    }
                }
