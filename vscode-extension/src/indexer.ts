...
export interface Definition {
  name: string;
  kind: 'request' | 'auth' | 'fragment' | 'flow' | 'env';
  file: string;
  line: number;
  method?: string;
  url?: string;
...
export class FlowSpecIndex {
  ...
  lookup(name: string, kind?: string): Definition | undefined {
    ...
    if (this.definitions.has(reqKey)) return this.definitions.get(reqKey);
    // Try auth
    const authKey = `auth:${name}`;
    if (this.definitions.has(authKey)) return this.definitions.get(authKey);
    // Try fragment
    const fragKey = `fragment:${name}`;
    if (this.definitions.has(fragKey)) return this.definitions.get(fragKey);
